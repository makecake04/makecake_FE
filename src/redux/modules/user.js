...
import KakaoLogin from "react-kakao-login";
  ...
  const setUsername = createAction(SET_USERNAME, (username, isTrue) => ({ username, isTrue }));
  const addPassword = createAction(ADD_PASSWORD, password => ({ password} ));
  const addPasswordCheck = createAction(ADD_PASSWORDCHECK, passwordCheck => ({ passwordCheck }));
