...
//import css
import { NotiWrap, Container, BlackBackButton, HeaderText, Line, FixListBox, PersonalListBox, ImgWrap, CakeIcon, FixTextWrap, PersoNalTextWrap, MainText, SubText, TimeText, NotNoti, BigCakeIcon, NotNotiText, 
         LoginText, LoginBox, NotiCheckDot, NotiHeart, NotiComment } from "./style";
  ...
  return (
    ...
    {token ? <>
        {notiFixList.map((v, idx) => {
          return (
            <FixListBox key={idx} onClick={() => {window.open(v.redirectUrl)}}>
              <ImgWrap>
                <CakeIcon/>
              </ImgWrap>
              <FixTextWrap>
          ...
          {notiPersonalList.length === 0 ? 
                    ...
                    return (
                      <PersonalListBox key={idx} onClick={() => {window.open(v.redirectUrl)}}>
                        <ImgWrap>
          ...
          <LoginBox>
            ...
            <LoginText onClick={() => {navigate('/')}}>로그인하러 가기</LoginText>
          </LoginBox>
        </NotiWrap>  
      }
  </>
